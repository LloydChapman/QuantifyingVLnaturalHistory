...
## Classify ages
# ## adult vs child
# data_long$age_groups<-0
# data_long$age_groups[data_long$age_m<18]<-1
## different groups
data_long$age_groups[data_long$age_m<3]<-1
data_long$age_groups[data_long$age_m>=3]<-2
data_long$age_groups[data_long$age_m>=15]<-3
data_long$age_groups[data_long$age_m>45]<-4
...
# Asymptomatic pre-KA
state2_idx<-(!active_KA & posE & negL & !end_of_trtmnt & pre_KA & !post_KA)

# Asymptomatic without developing KA
state6_idx<-(!active_KA & posE & negL & !end_of_trtmnt & !pre_KA & !post_KA)
...
# NEED TO THINK CAREFULLY ABOUT DEFINITION OF CENSORED STATES WITH EXTRA ASYMPT STATE
...
# State 2 or 6 or 4: for pre KA or no KA: either (E+ & missing LST) or (missing ELISA & L- & possibility of serodeconversion)
state92_idx<-(!active_KA & !end_of_trtmnt & ((posE & !hasL & !post_KA) 
                          | (!hasE & negL & last_ELISA_pstve_idx & !post_KA )))
# State 1 or 2 or 6: for preK or no K: missing ELISA, L- & no possibility of serodeconversion
state93_idx<-(!active_KA & !end_of_trtmnt & !hasE & negL & !last_ELISA_pstve_idx & !post_KA)

print(sum(state1_idx,na.rm=T))
print(sum(state2_idx,na.rm=T))
print(sum(state3_idx,na.rm=T))
print(sum(state4_idx,na.rm=T))
print(sum(state5_idx,na.rm=T))
print(sum(state6_idx,na.rm=T))
print(sum(state91_idx,na.rm=T))
print(sum(state92_idx,na.rm=T))
print(sum(state93_idx,na.rm=T))
...
obstype[state1_idx|state2_idx|(state4_idx & !end_of_trtmnt)|state6_idx|state91_idx|state92_idx|state93_idx]<-1
...
# Uncomment to write output to data_long.csv
# write.csv(data_long,"data_long_extra_asympt_state_all_deaths_and_relapse.csv")
